...
// Responsive design smoke check across the SvelteKit examples app.
//
...
//
// This is a smoke check, not a pixel-perfect responsive audit. Real
// responsive verification still requires manual layout review and
...
async function expectNoHorizontalOverflow(page: import('@playwright/test').Page, label: string) {
  ...
  // Allow a 2-pixel jitter for sub-pixel rounding by the browser.
  ...
  test.describe(`responsive: ${vp.name.trim()} (${vp.width}x${vp.height})`, () => {
    ...
    for (const route of routes) {
      test(route, async ({ page }) => {
        ...
        // <main> is rendered (some demo pages render nested <main> elements
        // inside the live-demo block; the page-level main is the first one)
