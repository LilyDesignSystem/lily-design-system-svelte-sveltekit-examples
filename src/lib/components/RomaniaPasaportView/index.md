...
A headless read-only display for Romania's Paşaport. Format: 8 characters: positions 1-2 [A-Z] and positions 3-8 [0-9].
...
- Renders `<span class="romania-pasaport-view ...">{value}</span>`.
- Sets `aria-label={label}` for screen reader name.
- Renders the `value` prop as text content.

## When to use it

- Display a stored Romania Paşaport read-only.
- Summary lists, patient records, audit logs, detail views.

## When not to use it

- For editing, use `RomaniaPasaportInput`.
- For general text display, use a `<span>` directly.

## Headless

This component ships zero CSS. The consumer provides all styling targeting `.romania-pasaport-view`.
